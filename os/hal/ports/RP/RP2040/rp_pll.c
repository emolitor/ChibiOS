 ...
 * @brief   RP2040 PLL driver source.
 *
 * @addtogroup RP_PLL
 * @{
 */
...
void rp_pll_init(PLL_TypeDef *pll, uint32_t refdiv, uint32_t vco_freq,
  ...
  /* Check if PLL is already configured correctly and locked. */
  if ((pll->CS & PLL_CS_LOCK) &&
    ...
    /* PLL already correctly configured, do not disturb. */
    return;
  }
  ...
  /* Turn on PLL power (clear power-down bits). */
  pll->PWR &= ~(PLL_PWR_PD | PLL_PWR_VCOPD);

  /* Wait for PLL to lock. */
  while ((pll->CS & PLL_CS_LOCK) == 0U) {
    /* Spin. */
  }
