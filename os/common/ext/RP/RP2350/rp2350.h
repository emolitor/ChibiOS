...
/**
 * @brief   SIO (Single-cycle IO) peripheral.
 * @note    RP2350 interleaves base GPIO and GPIO_HI registers differently
 *          from RP2040. The base GPIO registers (GPIO_IN, GPIO_OUT, etc.)
 *          control GPIO 0-31, while GPIO_HI registers control GPIO 32-47
 *          plus QSPI and USB pins.
 */
typedef struct {
  __I  uint32_t         CPUID;
  __I  uint32_t         GPIO_IN;            /**< Input for GPIO 0-31.      */
  __I  uint32_t         GPIO_HI_IN;         /**< Input for GPIO 32-47.     */
  __I  uint32_t         resvdC;
  __IO uint32_t         GPIO_OUT;           /**< Output for GPIO 0-31.     */
  __IO uint32_t         GPIO_HI_OUT;        /**< Output for GPIO 32-47.    */
  __IO uint32_t         GPIO_OUT_SET;       /**< Atomic set GPIO 0-31.     */
  __IO uint32_t         GPIO_HI_OUT_SET;    /**< Atomic set GPIO 32-47.    */
  __IO uint32_t         GPIO_OUT_CLR;       /**< Atomic clear GPIO 0-31.   */
  __IO uint32_t         GPIO_HI_OUT_CLR;    /**< Atomic clear GPIO 32-47.  */
  __IO uint32_t         GPIO_OUT_XOR;       /**< Atomic XOR GPIO 0-31.     */
  __IO uint32_t         GPIO_HI_OUT_XOR;    /**< Atomic XOR GPIO 32-47.    */
  __IO uint32_t         GPIO_OE;            /**< Output enable GPIO 0-31.  */
  __IO uint32_t         GPIO_HI_OE;         /**< Output enable GPIO 32-47. */
  __IO uint32_t         GPIO_OE_SET;        /**< Atomic set OE GPIO 0-31.  */
  __IO uint32_t         GPIO_HI_OE_SET;     /**< Atomic set OE GPIO 32-47. */
  __IO uint32_t         GPIO_OE_CLR;        /**< Atomic clear OE GPIO 0-31.*/
  __IO uint32_t         GPIO_HI_OE_CLR;     /**< Atomic clear OE GPIO 32-47*/
  __IO uint32_t         GPIO_OE_XOR;        /**< Atomic XOR OE GPIO 0-31.  */
  __IO uint32_t         GPIO_HI_OE_XOR;     /**< Atomic XOR OE GPIO 32-47. */
 ...
 * @note    RP2350 moved TICK to separate TICKS peripheral.
 */
typedef struct {
  __IO uint32_t         CTRL;
  __O  uint32_t         LOAD;
...
#define TIMER                             TIMER0  /* Alias for compatibility with RP2040 code */
...
/**
 * @name    PSM bits definitions
 * @note    RP2350 has different PSM bit assignments than RP2040.
 * @{
 */
