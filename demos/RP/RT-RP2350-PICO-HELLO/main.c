...
/*
 * Simple LED blink demo for Raspberry Pi Pico (RP2040).
 * This is a minimal single-threaded example that blinks the onboard LED
 * and outputs "Hello RP2350!" over serial when the LED turns on.
 */
...
int main(void) {
   ...
   * Note: stBind() is now called automatically by __port_platform_init()
   * in hal_lld_rp2350_port.c during chSysInit().
   */
  halInit();
  chSysInit();
  ...
  /*
   * Main loop - blink the LED every 500ms and output "Hello RP2350!" when LED turns on.
   */
  while (true) {
    static const char msg[] = "Hello RP2350!\r\n";
